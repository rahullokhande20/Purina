...
final class ChannelsViewController: UIViewController, CBPeripheralDelegate, ChartViewDelegate, UITextFieldDelegate {

    private enum Constants {
        static let navigationTitle = "Multi Channel Doppler"
    ...
    let hexGainArray = ["00","01","02","03","04","05","06","07","08","09","0A","0B","0C"]
    let dropDownGainArray = ["00","01","02","03","04","05","06","07","08","09","10","11","12"]
    ...
    @IBAction func writeGainAction(){
        ...
        peripheral.writeValue(response!, for: char, type: CBCharacteristicWriteType.withoutResponse)
        peripheral.setNotifyValue(true, for: char)
        isGainWritten = true
    ...
    @objc func txtButtonAction(_ sender: UIButton){
        ...
        if txtButton.tag == 0{
            ...
            if breedTextfield.text?.count == 0 {
                ...
                fileNameString = "\(breedTextfield.text ?? "")_\(locationTextfield.text ?? ""):\(dateString)-MCD.txt"
                fileNameString = fileNameString.replacingOccurrences(of: " ", with: "_")

                writePacket = "latency : \(startDelay ?? 0.000)" + "\n" + "PS , Data , PS State , Gain , Channel , Activity"
                let readString = "\(self.readGainLabel.text ?? "")0000"
            ...
            if hexArray.count == 7{
                ...
                DispatchQueue.main.async {
                    ...
                    if self.serviceType == "Gain"{
                        ...
                        if let value = UInt8(hexArray[5], radix: 16) {
                            print(value)
                ...
                if newWritePacket{
                    ...
                    let writePacket = writePacketGain + writePacketCurrent + "0000"
                    appendToSavedFile("---------\(writePacket)\n")
        ...
        for i in hexArray{
            ...
            if !self.stopSaving{
                ...
                if let index = selectedSwitch{
                    valueString = "\(currNo), \(decimalValue), \(isModified ? 1 : 0), \(readGainLabel.text ?? "-1"), \(channelArray[index]), \(activityIndexString ?? "")"
                }else{
                    valueString = "\(currNo), \(decimalValue), \(isModified ? 1 : 0), \(readGainLabel.text ?? "-1"), 0, \(activityIndexString ?? "")"
                }
...
extension ChannelsViewController : UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
